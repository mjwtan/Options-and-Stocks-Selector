...
class RiskResult:
    ...
    fallback_reason: Optional[str] = None  # set when montecarlo failed and this is the analytic fallback (system-spec.md S15.5)


def _analytic_result(sigma_p_analytic: float, cfg: RiskConfig, t0: float, fallback_reason: Optional[str] = None) -> RiskResult:
...
def compute_risk_scalar(weights: "pd.Series", Sigma_ann: np.ndarray, cfg: RiskConfig) -> RiskResult:
    ...
    A montecarlo failure falls back to the analytic engine rather than
    crashing the run (system-spec.md S15.5: "MC engine fails or exceeds
    time budget -> Fall back to RISK_ENGINE=analytic, log loudly"). The
    analytic path itself has no failure mode worth catching - it's a
    closed-form formula on inputs already validated upstream."""
    w = weights.values
    ...
    if cfg.risk_engine == "analytic":
        return _analytic_result(sigma_p_analytic, cfg, t0)
    ...
    try:
        ...
        print(f"  WARNING: montecarlo risk engine failed ({e!r}) - falling back to --risk-engine analytic for this run (S15.5)")
        return _analytic_result(sigma_p_analytic, cfg, t0, fallback_reason=f"montecarlo failed: {e}")
