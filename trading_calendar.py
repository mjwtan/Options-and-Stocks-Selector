...
Named `trading_calendar.py` rather than `calendar.py` (as supplied in
mdinstructions/calendar.py) deliberately: a top-level `calendar.py` shadows
the stdlib `calendar` module for anything importing from the repo root.
`risk/calendar.py` avoids this only because it lives inside a package;
there is no such protection at the top level, so the module is renamed
here instead. Public surface is otherwise unchanged from the supplied file.

Deviation from the supplied file: it constructs `self.dc = ql.Actual252()`,
which does not exist in the installed QuantLib binding (1.43) — confirmed
by running it; `AttributeError: module 'QuantLib' has no attribute
'Actual252'`. The correct business-day-based 252 day counter is
`ql.Business252(calendar)`, which takes the exchange calendar so that
`yearFraction` counts actual trading sessions rather than calendar days.
Used here instead.
...
class TradingCalendar:
    ...
    def __init__(self, market: str = "NYSE"):
        ...
        self.dc = ql.Business252(self.cal)
    ...
    def year_fraction(self, start, end) -> float:
        """Business/252 year fraction (actual trading sessions between start
        and end, divided by 252). Use instead of hardcoding /252."""
    ...
    def assert_fresh(self, latest_bar_date, run_date=None) -> None:
        """Abort if the most recent bar is older than the previous session.

        The supplied version of this check required an exact match
        (latest bar == previous session), which is only correct for a
        strictly pre-market run. Run this script in the afternoon/evening
        - the normal case for a manually-triggered weekly rebalance - and
        the data provider may already have *today's* daily bar, which is
        fresher than "the previous session", not staler. Exact-match then
        raises on perfectly good data. Accept latest_bar_date >= the
        previous session instead; only reject when it's actually behind.
        """
        ...
        if actual < expected:
            raise RuntimeError(
                f"stale bars: latest is {actual}, expected at least {expected}"
            )
